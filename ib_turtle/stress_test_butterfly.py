...
def run_stress_test(csv_path="spx_vix_hourly.csv", wing_width=10, slippage=0.10, strike_step=5):
    ...
    haircuts = [0.0, 0.50, 1.00, 1.50, 2.00]
    results = []
    ...
    for haircut in haircuts:
        ...
        for idx, row in data.iterrows():
            ...
            actual_premium = theoretical_premium - slippage - haircut
            
            # Use a realistic relative minimum premium target for entry (e.g. 5.50 after haircut)
            # This ensures we get filled even on lower premium days
            if actual_premium >= 5.00:
                trades_count += 1
    ...
    print("\n" + "="*85)
    print("      STRESS TEST: PREMIUM HAIRCUTS FOR 1:30 PM SPX IRON BUTTERFLY")
    print("="*85)
    print(res_df.to_string(index=False, formatters={
        "Premium Haircut": "${:,.2f}".format,
        "Trades": "{:d}".format,
